...
from pyspark.sql.types import StructType, StructField, IntegerType, DoubleType, ArrayType, LongType
...
class MyRecommender:
    ...
    def __init__(self, seed=None, penalty=None, C=10):
        ...
        if self.penalty == 'l1' or self.penalty == 'l2':
            self.model = LogisticRegression(random_state=self.seed, max_iter=1000, penalty=self.penalty, solver='liblinear', C=self.C)
        elif self.penalty == 'elasticnet':
            self.model = LogisticRegression(random_state=self.seed, max_iter=1000, penalty=self.penalty, solver='saga', l1_ratio=0.5,C=self.C)
        else:
...
def run_recommender_analysis():
    ...
    recommenders = [
        ...
        MyRecommender(seed=42, penalty='l1'),
        MyRecommender(seed=42, penalty='l2'),
