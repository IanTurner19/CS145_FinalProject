...
# GCN Recommender Implementation for RecSys
import torch
import torch.nn as nn
...
class MyGCNRecommender:
    def __init__(self, embedding_dim=64, epochs=10, lr=0.01, dropout=0.3):
    ...
    def fit(self, log, user_features=None, item_features=None):
        ...
        if item_features is not None:
            item_prices_df = item_features.select("item_idx", "price").toPandas()
            df = df.merge(item_prices_df, on="item_idx", how="left")

        self.num_users = df['user_idx'].max() + 1
        self.num_items = df['item_idx'].max() + 1
        self.num_nodes = self.num_users + self.num_items

        user_nodes = torch.tensor(df['user_idx'].values)
        item_nodes = torch.tensor(df['item_idx'].values) + self.num_users
        edge_index = torch.stack([
            torch.cat([user_nodes, item_nodes]),
            torch.cat([item_nodes, user_nodes])
        ], dim=0)

        edge_set = set(zip(user_nodes.tolist(), item_nodes.tolist()))
        neg_samples = []
        while len(neg_samples) < len(user_nodes):
            u = np.random.randint(0, self.num_users)
            i = np.random.randint(self.num_users, self.num_nodes)
            if (u, i) not in edge_set:
                neg_samples.append((u, i))

        pos_pairs = torch.tensor(list(zip(user_nodes.tolist(), item_nodes.tolist())))
        neg_pairs = torch.tensor(neg_samples)
        ...
        for _ in range(self.epochs):
            optimizer.zero_grad()
            dropped_edge_index = dropout_adj(edge_index, p=self.dropout, force_undirected=True)[0]
            embeddings = self.model(dropped_edge_index)
    ...
    def predict(self, log, k, users, items, user_features=None, item_features=None, filter_seen_items=True):
        ...
        recs = []
        item_prices_tensor = torch.tensor([self.item_prices.get(i, 1.0) for i in items['item_idx']])
        for uid in users['user_idx']:
            user_emb = self.embeddings[uid]
            item_indices = torch.tensor(items['item_idx'].values) + self.num_users
            item_embs = self.embeddings[item_indices]
            scores = torch.sigmoid((user_emb * item_embs).sum(dim=1)) * item_prices_tensor
            top_k_indices = torch.topk(scores, k).indices
            top_items = items.iloc[top_k_indices.numpy()]
            for idx, row in top_items.iterrows():
                recs.append({
                    'user_idx': uid,
                    'item_idx': row['item_idx'],
                    'relevance': float(scores[top_k_indices[top_items.index.get_loc(idx)]])
                })
...
def run_recommender_analysis():
    ...
    recommenders = [
       ...
       MyGCNRecommender()  # Add your custom recommender here
    ]
    recommender_names = ["SVM", "Random", "Popularity", "ContentBased", "GCN"]
