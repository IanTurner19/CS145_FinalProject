...
"""
class MyRecommender:
    def __init__(self, seed=None, order=3, smoothing=1):
        self.seed = seed
        self.max_order = order  # Try orders 1, 2, 3 with backoff
        self.smoothing = smoothing
        self.user_history = defaultdict(list)
        self.ngram_counts = {n: defaultdict(Counter) for n in range(1, self.max_order + 1)}
    ...
    def fit(self, log, user_features=None, item_features=None):
        ...
        for user_id, user_log in log.groupby("user_idx"):
            item_seq = user_log['item_idx'].tolist()
            self.user_history[user_id] = item_seq

            for n in range(1, self.max_order + 1):
                for i in range(len(item_seq) - n):
                    prefix = tuple(item_seq[i:i + n])
                    next_item = item_seq[i + n]
                    self.ngram_counts[n][prefix][next_item] += 1

        self.item_counts.update(log['item_idx'])
    ...
    def predict(self, log, k, users, items, user_features=None, item_features=None, filter_seen_items=True):
        ...
        if log is not None:
            log = log.toPandas()
            log['timestamp'] = log.index
            for uid, group in log.groupby('user_idx'):
                new_items = group.sort_values("timestamp")['item_idx'].tolist()
                self.user_history[uid].extend(new_items)

        all_items = set(items['item_idx'])
        recs = []
        ...
        for uid in users['user_idx'].unique():
            ...
            for n in reversed(range(1, self.max_order + 1)):
                if len(history) < n:
                    continue
                prefix = tuple(history[-n:])
                pred_counts = self.ngram_counts[n].get(prefix, Counter())
                total = sum(pred_counts.values()) + self.smoothing * len(all_items)

                for item in all_items:
                    count = pred_counts.get(item, 0)
                    prob = (count + self.smoothing) / total
                    scores[item] += prob * (0.5 ** (self.max_order - n))  # backoff weight

            if not scores:
                for item in all_items:
                    scores[item] = self.item_counts[item] / max(1, sum(self.item_counts.values()))

            if filter_seen_items:
                seen = set(history)
                scores = {item: score for item, score in scores.items() if item not in seen}

            item_df = items.set_index('item_idx').loc[list(scores.keys())][['price']]
            item_df['score'] = [scores[i] for i in item_df.index]
            item_df['expected_revenue'] = item_df['score'] * item_df['price']

            top_k = item_df.nlargest(k, 'expected_revenue')
            for item_id, row in top_k.iterrows():
                recs.append({
                    'user_idx': uid,
                    'item_idx': item_id,
