...
class MyRecommender:
    ...
    def fit(self, log, user_features=None, item_features=None):
        log_pd = log.toPandas()
        uf = user_features.toPandas()
        itf = item_features.toPandas()

        self.user_features_df = uf
        self.item_features_df = itf

        train = log_pd.merge(uf, on='user_idx').merge(itf, on='item_idx')
        X = train.drop(columns=['user_idx', 'item_idx', 'relevance'])
        X = pd.get_dummies(X)
        y = train['relevance']
    ...
    # Convert Spark DataFrames to pandas
        ...
        pairs = pd.merge(users_pd.assign(key=1), items_pd.assign(key=1), on='key').drop(columns='key')

        # Merge with user and item features
        merged = pairs.merge(uf, on='user_idx').merge(itf, on='item_idx')

        # Backup the original price
        price_col = next((col for col in merged.columns if 'price' in col.lower()), None)
        if price_col is None:
            raise ValueError("No column with 'price' found in merged dataframe.")
        merged['orig_price'] = merged[price_col]

        # One-hot encode
        X = pd.get_dummies(merged.drop(columns=['user_idx', 'item_idx', 'orig_price']))
        ...
        merged['prob'] = preds
        merged['relevance'] = merged['prob'] * merged['orig_price']

        # Filter out seen items
        if filter_seen_items and log is not None:
            seen = log.select('user_idx', 'item_idx').toPandas()
            merged = pd.merge(merged, seen, on=['user_idx', 'item_idx'], how='left', indicator=True)
            merged = merged[merged['_merge'] == 'left_only'].drop(columns=['_merge'])

        # Sort and rank by expected revenue
        merged = merged.sort_values(by=['user_idx', 'relevance'], ascending=[True, False])
        topk = merged.groupby('user_idx').head(k)

        return spark.createDataFrame(topk[['user_idx', 'item_idx', 'relevance']])
